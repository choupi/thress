...
dir_e computeDir(Grid &myGrid, char NT, int* m_grabBag, int dep, float* result)
{
	...
	for(int d=0;d<4;d++) {
	  ...
	  for(int i=0;i<RUNCNT;i++) {
		...
		float ret[4]={0};
		if(dep>0) {
			computeDir(tg, getNT(tg,tmp_grabBag), tmp_grabBag, dep-1, ret);
			for(int j=0;j<4;j++) dm[d][j]+=ret[j];
		} else {
	...
	if(dep==0) { for(int j=0;j<4;j++) result[j]=dm[ds][j]; }
...
void PlayNRounds(int n){
    ...
    for(int i = 0;i < n;i++){    
        ...
        while(!isGameOver){
			...
			dir=computeDir(myGrid, h, m_grabBag, 1, NULL);
